...
class UNet_feature_dropout(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_feature_dropout, self).__init__()
...
class UNet_complementary_feature_dropout(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_complementary_feature_dropout, self).__init__()

        # 保持你的参数定义
        params = {'in_chns': in_chns,
                  'feature_chns': [16, 32, 64, 128, 256], # 示例通道数
                  'dropout': [0.05, 0.1, 0.2, 0.3, 0.5], # 这些内部 dropout 可能不再需要，或者可以保留用于其他层
                  'class_num': class_num,
                  'bilinear': False,
                  'acti_func': 'relu'}
        self.params = params # 存储参数以便访问

        self.encoder = Encoder(params)
        self.decoder = Decoder(params)

    def get_bottleneck_channels(self):
        """返回瓶颈层的通道数"""
        return self.params['feature_chns'][-1]
    ...
    def forward(self, x, dropout_mask=None):
        ...
        Args:
            x (torch.Tensor): 输入张量。
            dropout_mask (torch.Tensor, optional):
                应用到瓶颈层特征的 Dropout 掩码。
                形状应为 (batch_size, bottleneck_channels, 1, ...)
                如果为 None，则不应用外部 Dropout。
                Defaults to None.

        Returns:
            torch.Tensor: 模型的输出分割图。
        """
        # 1. 通过编码器获取特征图列表
        # 假设 self.encoder(x) 返回一个列表，其中最后一个元素是瓶颈层特征
        features = self.encoder(x)

        # 2. 应用外部 Dropout (如果提供了掩码)
        if dropout_mask is not None:
            bottleneck_feature = features[-1] # 获取最深的特征图

            # 检查掩码形状是否兼容 (基本检查)
            if bottleneck_feature.shape[0] == dropout_mask.shape[0] and \
               bottleneck_feature.shape[1] == dropout_mask.shape[1] and \
               bottleneck_feature.dim() == dropout_mask.dim():

                # 应用掩码 (元素级乘法)
                masked_bottleneck_feature = bottleneck_feature * dropout_mask

                # 创建一个新的特征列表，用掩码后的特征替换原来的瓶颈特征
                # 注意：直接修改 features[-1] = masked_bottleneck_feature 也可以，
                # 但创建副本有时更安全，取决于 Encoder/Decoder 的实现。
                features_for_decoder = list(features[:-1]) + [masked_bottleneck_feature]
            else:
                print(f"Warning: Dropout mask shape {dropout_mask.shape} mismatch with "
                      f"bottleneck feature shape {bottleneck_feature.shape}. Skipping dropout.")
                features_for_decoder = features # 使用原始特征
        else:
            # 如果没有提供掩码，正常传递特征
            features_for_decoder = features

        # 3. 通过解码器生成输出
        # 确保 Decoder 可以处理特征列表
        output = self.decoder(features_for_decoder)
...
class Decoder_BCP(nn.Module):
    def __init__(self, params):
        super(Decoder_BCP, self).__init__()
        self.params = params
        self.in_chns = self.params['in_chns']
        self.ft_chns = self.params['feature_chns']
        self.n_class = self.params['class_num']
...
class UNet_2d(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder_BCP(params)
        dim_in = 16
        feat_dim = 32
