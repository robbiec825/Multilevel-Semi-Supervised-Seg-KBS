...
class SwinTransformerSys_fp(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    # 使用原始编码器获取特征
        ...
        if need_fp:
            # 对 bottleneck 特征和跳跃连接应用 dropout
            x_dropout = nn.Dropout(p=0.5)(x)
            x_downsample_dropout = [nn.Dropout(p=0.5)(feat) for feat in x_downsample]
            
            # 使用原始特征和dropout后的特征分别解码
            output1 = self.forward_up_features(x, x_downsample)
            output1 = self.up_x4(output1)
            
            output2 = self.forward_up_features(x_dropout, x_downsample_dropout)
            output2 = self.up_x4(output2)
